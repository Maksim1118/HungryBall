...
Options::Options()
{
	...
	//SplitedKeyCode = SpliteCode;
	
	//FeededKeyCode = FeedCode;
	...
	/*if (FeededKeyCode == Keyboard::Key::E)
	{
...
void Options::moveSlider(int mouseX)
{
	...
	//music.setVolume(volume);
	cout << volume << endl;
	//cout  << _LineMus.getGlobalBounds().width << endl;
}
bool Options::PosSlider(float x, float y)
{
...
bool Options::PosBox(float x, float  y)
{
...
bool Options::PosKey(Text text, float x, float  y)
{
...
bool Options::PosButtonClose(float x, float y)
{
...
bool Options::PosButtonSave(float x, float y)
{
...
void Options::Saved()
{
	...
	File << volume << " " << SplitedKeyCode << " " << FeededKeyCode  << " " << _isBoxPressed; //<< SplitedKeyCode;
	File.close();
}
...
Text Options::CreateTitle(const wstring text, int size)
{
	Text _Text;
	_Text.setFont(_Font);
